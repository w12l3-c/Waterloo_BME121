...
int main(){
    ...
    BmodeClass *currentScanline = scanlineHead; // create a new pointer that points to the current scanline in the linked list
    dataBuffer *currData = dataHead;   // create a new pointer that points to the current scanline data in the linked list
    cout << sizeof(dataHead) << endl;   
    for (int i = 1; i < parameters->getNumScanline(); i++){
        currData = currData->next;  // update the pointer and move the data to the next scanline
        currentScanline->next = new BmodeClass(parameters, currData, i);    // create a new bmoded scanline for the next item in the linked list
        currentScanline = currentScanline->next;    // update the pointer and move the current scanline to the next scanline
        currentScanline->next = NULL;   // put NULL on the next item to indicate the end of linked list
    }
...
/*
Compile command:
g++ ImageDisplay.cpp EBO.cpp VAO.cpp VBO.cpp Texture.cpp shaderClass.cpp glew.c -Llib -lglfw3 -framework Cocoa -framework OpenGL -framework IOKit -Iinclude -DGLEW_STATIC Bmode.cpp dataBuffer.cpp BmodeClass.cpp imageParam.cpp -o bmode
*/